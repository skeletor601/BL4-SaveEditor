...
# Master Search (Scarlett HTML + DB). Scarlett loads universal_parts_db.json first, else community_parts_db.json
master_search_files = []
for _path in ["master_search/scarlett.html", "master_search/db/universal_parts_db.json", "master_search/db/community_parts_db.json"]:
...
a = Analysis(
    ...
    ] + {enhancement_files} + {weapon_files} + {grenade_files} + {shield_files} + {repkit_files} + {heavy_files} + {master_search_files} + {root_files},
    hiddenimports=[
        'PIL',
        'pandas',
        'PIL.Image',
