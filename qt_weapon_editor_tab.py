...
class WeaponEditorTab(QtWidgets.QWidget):
    ...
    def load_data(self, lang='en-US'):
        try:
            ...
            # Helper to get path with fallback (raises FileNotFoundError if neither exists)
            def get_path(base_name):
                ...
                raise FileNotFoundError(
                    ...
                    "Run Master Search / Check for DB updates to refresh community data."
                )
    ...
    def update_language(self, lang):
        ...
        # Save current state
        current_decoded = self.serial_decoded_entry.toPlainText() if hasattr(self, 'serial_decoded_entry') else ""
        ...
        if current_decoded:
             self.serial_decoded_entry.setPlainText(current_decoded) # Set text first so it's available if parse fails
    ...
    def create_widgets(self):
        ...
        if flags:
            ...
            self.flag_combo.addItems(["1 (Normal)", "3 (Favorite)", "5 (Junk)", "17 (Group 1)", "33 (Group 2)", "65 (Group 3)", "129 (Group 4)"])
        ...
        layout.addWidget(act_frame, 3, 0)
        ...
        self.serial_decoded_entry.textChanged.connect(lambda: self.handle_decoded_change())
    ...
    def handle_b85_change(self, text):
        # Keep this responsive for both user edits and programmatic loads.
        # We rely on is_handling_change + signal blocking to prevent loops.
    ...
    def handle_decoded_change(self, text=None):
        if self.is_handling_change:
            return
        if text is None:
            # AutoGrowPlainTextEdit/QPlainTextEdit emits textChanged() with no args
            text = self.serial_decoded_entry.toPlainText() if hasattr(self.serial_decoded_entry, 'toPlainText') else self.serial_decoded_entry.text()
    ...
    def load_from_item(self, item: dict):
        """Load an inventory item into the weapon editor and fully populate Base85 + Decoded.

        This bypasses focus-based gating so 'Edit in Wpn Edit' from Items works reliably.
        """
        serial_b85 = (item or {}).get("serial", "") or ""
        decoded = (item or {}).get("decoded_full", "") or (item or {}).get("decoded", "") or ""

        # If decoded isn't provided, derive it from Base85.
        if serial_b85 and not decoded:
            decoded_str, _, err = bl4f.decode_serial_to_string(serial_b85)
            if not err:
                decoded = decoded_str

        # If Base85 isn't provided but decoded is, derive Base85.
        if decoded and not serial_b85:
            enc, err = b_encoder.encode_to_base85(decoded)
            if not err:
                serial_b85 = enc

        # Populate fields without triggering recursive handlers.
        self.is_handling_change = True
        try:
            self.serial_b85_entry.blockSignals(True)
            self.serial_b85_entry.setPlainText(str(serial_b85))
            self.serial_b85_entry.blockSignals(False)

            self.serial_decoded_entry.blockSignals(True)
            self.serial_decoded_entry.setPlainText(str(decoded))
            self.serial_decoded_entry.blockSignals(False)
        finally:
            self.is_handling_change = False

        # Drive the rest of the UI from decoded (single source of truth).
        if decoded:
            self.parse_and_display_weapon(decoded)
            # Ensure both fields are in sync after parsing.
            self.update_b85_from_decoded()
            self.serial_b85_entry.setReadOnly(True)
            self.update_weapon_btn.setEnabled(True)
        else:
            self.clear_all_fields(clear_b85=True)

    def update_b85_from_decoded(self):
        decoded_str = self.serial_decoded_entry.toPlainText()
        if not decoded_str: return
        new_b85, err = b_encoder.encode_to_base85(decoded_str)
        if not err:
            self.serial_b85_entry.blockSignals(True)
            self.serial_b85_entry.setPlainText(new_b85)
            self.serial_b85_entry.blockSignals(False)
    ...
    def _parse_component_string(self, component_str):
        ...
        for match in re.finditer(r'\{(\d+)(?::(\d+|\[[\d\s]+\]))?\}|\"c\",\s*(\d+)', component_str):
            ...
            if match.group(3): part_data.update({'type': 'skin', 'id': int(match.group(3))})
                ...
                if inner:
                    if '[' in inner:
                        part_data.update({'type': 'group', 'id': outer_id, 'sub_ids': [int(sid) for sid in inner.strip('[]').split()]})
                    else:
                        # In this editor, elemental parts are the special prefix {1:XX}.
                        # Other prefixes like {10:58} are *not* elemental — they mean the part is from another manufacturer/type.
                        if outer_id == 1:
                            part_data.update({'type': 'elemental', 'id': outer_id, 'sub_id': int(inner)})
                        else:
                            part_data.update({'type': 'part', 'mfg_id': outer_id, 'id': int(inner)})
    ...
    def update_decoded_from_ui(self):
        ...
        current_decoded = self.serial_decoded_entry.toPlainText()
        ...
        try:
            ...
            if self.serial_decoded_entry.toPlainText() != updated_str: self.serial_decoded_entry.setPlainText(updated_str)
    ...
    def load_weapon_data(self, weapon_data):
        ...
        self.is_handling_change = True
        self.serial_b85_entry.setPlainText(weapon_data.get('serial', ''))
    ...
    def parse_and_display_weapon(self, decoded_str):
        try:
            ...
            m_matches = self.all_weapon_parts_df[self.all_weapon_parts_df['Manufacturer & Weapon Type ID'] == m_id]
            if m_matches.empty:
                # Not a weapon serial (likely grenade/shield/repkit/heavy/etc).
                self.is_handling_change = True
                self.manufacturer_entry.setText("—")
                self.item_type_entry.setText("Not a Weapon Serial")
                ...
                QtWidgets.QMessageBox.information(
                    self,
                    self.get_localized_string("parse_error"),
                    "That code isn't a weapon serial, so Wpn Edit can't parse it as a gun.\n\nUse the Grenade/Shield/Repkit/Heavies tabs for those item types."
                )
                return
            m_info = m_matches.iloc[0]
            self.is_handling_change = True
    ...
    def display_parts(self, manufacturer_id):
        ...
        for i, part_info in enumerate(self.parts_data):
            ...
            if part_info.get('type') == 'group':
                frame = self._create_collapsible_part_frame(part_info, i)
            else:
                mfg_for_part = part_info.get('mfg_id', manufacturer_id)
                frame = self._create_simple_part_frame(part_info, mfg_for_part, i)
    ...
    def _create_simple_part_frame(self, part_info, m_id, index):
        ...
        part_id = part_info.get('id'); info = {'type': "Unknown", 'str': "Error", 'stat': ""}
    ...
    def _create_collapsible_part_frame(self, part_info, index):
        ...
        group_id = part_info.get('id', 0); mfg_name, is_known = "Unknown Manufacturer", False
        try:
        ...
        header_layout.addWidget(toggle_btn, 0, 0); header_layout.addWidget(QtWidgets.QLabel(f"Fold Group: {part_info['raw']} {'(unknown)' if not is_known else ''}"), 0, 1)
        ...
        for sub_id in part_info.get('sub_ids', []):
            ...
            p_type, p_str, p_stat = "Unknown", "Unable to parse", ""
    ...
    def regenerate_ui_and_serial(self):
        ...
        self.serial_decoded_entry.setPlainText(f"{header_part.strip()}|| {new_component_str}")
        self.display_parts(m_id)

    def force_refresh_parts(self):
        if not (decoded_str := self.serial_decoded_entry.toPlainText()):
    ...
    def refresh_backpack_items(self):
        ...
        for weapon in filtered:
            try:
                self.main_app.log(f"Processing backpack weapon, serial: {weapon.get('serial', 'N/A')}")
                header, component = weapon.get('decoded_full', '').split('||', 1)
                self.main_app.log("  - Successfully split header and components")
                
                m_id = int(header.strip().split('|')[0].strip().split(',')[0])
                self.main_app.log(f"  - Parsed manufacturer ID: {m_id}")

                parsed_components = self._parse_component_string(component)
                self.main_app.log(f"  - Parsed {len(parsed_components)} components")

                _, name, _, _ = self._get_rarity_and_weapon_name(parsed_components, m_id)
                self.main_app.log(f"  - Retrieved weapon name: {name}")
                
                w_name = self.get_localized_string(name, name)
                disp_name = f"{weapon.get('manufacturer', 'Unknown')} {weapon.get('type', 'Unknown Item')} ({w_name})" if w_name not in ["N/A", "Unknown", "Unknown"] else f"{weapon.get('manufacturer', 'Unknown')} {weapon.get('type', 'Unknown Item')}"
                ...
                self.main_app.log(f"  - Created and added weapon button: {btn_text}")

            except Exception as e:
                self.main_app.log(f"Critical error while processing backpack weapon. Serial: {weapon.get('serial', 'Unknown')}, error: {e}")
                # 创建一个Error按钮以提供反馈
                error_btn = QtWidgets.QPushButton(f"Error: Unable to load weapon with serial {weapon.get('serial', 'Unknown')}")
    ...
    def update_weapon(self):
        ...
        new_serial, err = b_encoder.encode_to_base85(self.serial_decoded_entry.toPlainText().strip())
        if err:
    ...
    def add_new_weapon_to_backpack(self):
        new_decoded = self.serial_decoded_entry.toPlainText().strip()
        if not new_decoded:
    ...
    def open_add_part_window(self):
        if not self.serial_decoded_entry.toPlainText():
...
def _copy_skin_code(self):
    token = self.skin_combo.currentData()
    if not token:
        return
    QtWidgets.QApplication.clipboard().setText(self._formatted_skin_code(token))

def _copy_skin_token(self):
    token = self.skin_combo.currentData()
    if not token:
        return
    QtWidgets.QApplication.clipboard().setText(str(token))

def _paste_skin_into_deserialize(self):
    token = self.skin_combo.currentData()
    if not token:
        return
    self._append_skin_to_decoded(token)

def _append_skin_to_decoded(self, token: str):
    """Append formatted skin code to end of decoded serial, then re-encode Base85."""
    try:
        decoded = (self.serial_decoded_entry.toPlainText() or "").strip()
        if not decoded:
            QtWidgets.QMessageBox.warning(self, "No weapon loaded", "Load or paste a weapon first, then apply a skin.")
            return
        ...
        try:
            self.parse_and_display_weapon(new_decoded)
        except Exception:
            pass
    except Exception as e:
...
def _update_skin_preview(self):
    ...
    scaled = pix.scaled(self._skin_prev_w, self._skin_prev_h,
                        QtCore.Qt.AspectRatioMode.KeepAspectRatio,
                        QtCore.Qt.TransformationMode.SmoothTransformation)
    self._skin_pix_full = pix
...
def _open_skin_lightbox(self):
    ...
    scaled = pix.scaled(max_w, max_h, QtCore.Qt.AspectRatioMode.KeepAspectRatio, QtCore.Qt.TransformationMode.SmoothTransformation)
    lbl.setPixmap(scaled)
    lbl.setStyleSheet("QLabel{border:2px solid rgba(180,255,80,180); border-radius:14px; background: rgba(10,10,16,120); padding:10px;}")
...
WeaponEditorTab._copy_skin_code = _copy_skin_code
WeaponEditorTab._copy_skin_token = _copy_skin_token
WeaponEditorTab._paste_skin_into_deserialize = _paste_skin_into_deserialize
WeaponEditorTab._append_skin_to_decoded = _append_skin_to_decoded
WeaponEditorTab._update_skin_preview = _update_skin_preview
WeaponEditorTab._open_skin_lightbox = _open_skin_lightbox


# Backwards-compatible alias
QtWeaponEditorTab = WeaponEditorTab
