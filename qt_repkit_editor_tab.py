...
def load_repkit_data(lang='en-US'):
    """使用资源加载器加载修复套件数据。"""
    try:
...
class QtRepkitEditorTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if self.df_main is None or self.df_mfg is None:
            msg = self.ui_loc.get('dialogs', {}).get('load_error', "Error: Repkit data could not be loaded.")
    ...
    def rebuild_output(self, *args):
        ...
        try:
            ...
            self.raw_output_edit.setPlainText(final_string)
            ...
            self.b85_output_edit.setPlainText(f"Error: {err}" if err else encoded_serial)
    ...
    def _on_decoded_text_changed(self):
        if self.is_handling_change:
            return
        if hasattr(self.raw_output_edit, 'hasFocus') and self.raw_output_edit.hasFocus():
    ...
    def _on_b85_text_changed(self):
        if self.is_handling_change:
            return
        if hasattr(self.b85_output_edit, 'hasFocus') and self.b85_output_edit.hasFocus():
            self._manual_output_mode = True
