/**
 * Rexalz's Vault — private code storage for Rexalz's weapon/grenade codes.
...
const VAULT_PATH = persistPath("rexalz_vault.json");
...
export async function rexalzVaultRoutes(fastify: FastifyInstance, _opts: unknown) {
  fastify.get("/rexalz-vault", async (_request, reply) => { reply.send(loadVault()); });

  fastify.post("/rexalz-vault", async (request, reply) => {
    ...
    const author = String(body.author ?? "Rexalz").trim();
  ...
  fastify.delete("/rexalz-vault/:id", async (request, reply) => {
  ...
  fastify.patch("/rexalz-vault/:id", async (request, reply) => {
    const { id } = request.params as { id: string };
    const body = request.body as Record<string, unknown>;
    const entries = loadVault();
  ...
  const GRENADE_CODES_PATH = persistPath("rexalz_grenade_codes.json");
  ...
  fastify.get("/rexalz-grenade-codes", async (_request, reply) => { reply.send(loadGrenadeCodes()); });

  fastify.post("/rexalz-grenade-codes", async (request, reply) => {
    const body = request.body as Record<string, unknown>;
  ...
  fastify.delete("/rexalz-grenade-codes/:id", async (request, reply) => {
    const { id } = request.params as { id: string };
    const codes = loadGrenadeCodes();
