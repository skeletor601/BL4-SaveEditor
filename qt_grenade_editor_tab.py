...
def load_grenade_data(lang='en-US'):
    try:
...
class QtGrenadeEditorTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Guard to prevent recursive Base85<->Decoded updates
        self.is_handling_change = False
        # When the user manually edits either output box, we stop auto-overwriting
        # their text when clicking perk/part controls.
        ...
        if self.df_main is None:
            msg = self.ui_loc.get('dialogs', {}).get('load_error', "Error: Grenade data could not be loaded.")
    ...
    def _create_output_group(self, layout):
        ...
        # Weapon-tab style output editors: auto-growing, wrapped, scrollable, pasteable.
    ...
    def _connect_signals(self):
        ...
        # Base85 <-> Decoded sync (weapon-tab style)
    ...
    def _on_decoded_text_changed(self):
        ...
        decoded = self.raw_output_edit.toPlainText()
    ...
    def _on_b85_text_changed(self):
        ...
        if self.b85_output_edit.hasFocus():
            self._manual_output_mode = True
        b85 = self.b85_output_edit.toPlainText().strip()
        if not b85:
            return
        self.is_handling_change = True
        try:
