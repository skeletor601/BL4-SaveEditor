...
class QtClassModEditorTab(QWidget):
    ...
    def _create_output_group(self):
        ...
        self.base85_output = AutoGrowPlainTextEdit(min_lines=5, max_lines=20)
        ...
        self.full_string_output = AutoGrowPlainTextEdit(min_lines=1, max_lines=6)
    ...
    def update_string(self, *args):
        ...
        if not self.names_data or not self.name_combo.currentText():
            self.full_string_output.setPlainText("...")
            self.base85_output.setPlainText("...")
            return
        ...
        try:
            ...
            self.full_string_output.setPlainText(full_string)

            encoded_serial, error = b_encoder.encode_to_base85(full_string)
            if error:
                self.base85_output.setPlainText(self.ui_loc['dialogs']['coding_error'].format(error=error))
            else:
                self.base85_output.setPlainText(encoded_serial)
        except Exception as e:
            import traceback
            traceback.print_exc()
            self.full_string_output.setPlainText(self.ui_loc['dialogs']['gen_error'].format(error=e))
            self.base85_output.setPlainText("...")
    ...
    def _add_to_backpack(self):
        serial = self.base85_output.toPlainText()
        if not serial or "Error" in serial or "Error" in serial:
