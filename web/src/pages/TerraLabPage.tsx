...
function TerraVault() {
  ...
  const saveEdit = async () => {
    ...
    setEditSaving(true);
    try {
      const res = await fetchApi(`terra-vault/${editingId}`, {
        method: "PATCH",
        body: JSON.stringify({
            ...
            {editingId === e.id ? (
              <div className="space-y-2">
                ...
                <input value={editLabel} onChange={(ev) => setEditLabel(ev.target.value)} placeholder="Name"
                  className="w-full px-3 py-1.5 rounded border border-[var(--color-panel-border)] bg-[rgba(12,14,18,0.8)] text-[var(--color-text)] text-xs focus:border-amber-500 focus:outline-none" />
                <textarea value={editCode} onChange={(ev) => setEditCode(ev.target.value)} rows={3}
                  className="w-full px-3 py-1.5 rounded border border-[var(--color-panel-border)] bg-[rgba(12,14,18,0.8)] text-[var(--color-text)] text-[10px] font-mono resize-y focus:border-amber-500 focus:outline-none" />
                ...
                <div className="flex gap-2">
                  <button onClick={saveEdit} disabled={editSaving}
                    className="px-3 py-1.5 rounded bg-amber-500/20 border border-amber-500/40 text-amber-400 text-[10px] font-medium hover:bg-amber-500/30 disabled:opacity-50">{editSaving ? "Saving..." : "Save"}</button>
              ...
              <>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={() => startEdit(e)} className="text-[10px] text-[var(--color-text-muted)] hover:text-amber-400">Edit</button>
