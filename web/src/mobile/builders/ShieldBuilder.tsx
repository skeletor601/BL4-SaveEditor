...
export default function ShieldBuilder() {
  ...
  const elemOpts = useMemo<PickerOption[]>(() => (data?.element ?? []).map((e) => ({ value: String(e.partId), label: `${e.partId} - ${e.stat}` })), [data]);
  const fwOpts = useMemo<PickerOption[]>(() => (data?.firmware ?? []).map((f) => ({ value: String(f.partId), label: `${f.partId} - ${f.stat}` })), [data]);
  ...
  const enOpts = useMemo<PickerOption[]>(() => (data?.energyPerks ?? []).map((p) => ({ value: String(p.partId), label: `${p.partId} - ${p.stat}` })), [data]);
  const arOpts = useMemo<PickerOption[]>(() => (data?.armorPerks ?? []).map((p) => ({ value: String(p.partId), label: `${p.partId} - ${p.stat}` })), [data]);
  ...
  const clearAll = useCallback(() => {
    setRarity(""); legends.clear(); elements.clear(); fw.clear(); uniPerks.clear(); energyPerks.clear(); armorPerks.clear(); setCode("");
  ...
  return (
    <div>
      ...
      <PartChecklist label="Element" options={elemOpts} selected={elements.parts} onToggle={elements.toggle} onQtyChange={elements.setQty} />
      ...
      {shieldType === "Energy" && <PartChecklist label="Energy Perks" options={enOpts} selected={energyPerks.parts} onToggle={energyPerks.toggle} onQtyChange={energyPerks.setQty} />}
      {shieldType === "Armor" && <PartChecklist label="Armor Perks" options={arOpts} selected={armorPerks.parts} onToggle={armorPerks.toggle} onQtyChange={armorPerks.setQty} />}
