...
class SaveGameController:
    ...
    def decrypt_save(self, file_path: Path, user_id: str, custom_backup_dir: Optional[str] = None) -> Tuple[str, str, str]:
        ...
        if plain_data is not None and platform_id:
            ...
            # 如果两种方法都失败，则抛出详细Error
            error_msg = ("解密存档文件失败。这通常意味着:\n"
                         "1. Error的用户ID - 请确保您使用的是正确的Epic Games或Steam用户ID\n"
                         "2. 损坏的存档文件 - 存档文件可能已损坏\n"
                         "3. Error的存档文件 - 这可能不是一个有效的BL4存档文件\n\n"
                         f"Error详情: {error}")
    ...
    def remove_item(self, item: dict) -> bool:
        """Remove an item from the loaded save.

        Expects item dict from Items tab (contains 'original_path').
        """
        if not self.yaml_obj:
            return False
        original_path = item.get("original_path")
        return bl4f.remove_item_by_original_path(self.yaml_obj, original_path)
