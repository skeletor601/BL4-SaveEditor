...
from PyQt6.QtCore import pyqtSignal, Qt, QModelIndex, QEvent
...
class QtItemsTab(QWidget):
    ...
    duplicate_item_requested = pyqtSignal(dict)  # emit full item dict
    remove_item_requested = pyqtSignal(dict)     # emit full item dict
    edit_item_requested = pyqtSignal(dict)       # emit full item dict
    ...
    def _create_details_pane(self, layout: QVBoxLayout):
        self.detail_fields: Dict[str, Any] = {}  # QLineEdit for 等级, QPlainTextEdit for 序列/解码ID
        ...
        for key in field_keys:
            if key == "等级":
                ...
                # 序列 (serial) and 解码ID (decoded): use QPlainTextEdit so full string is visible and select-all/copy work
                widget = QPlainTextEdit()
                widget.setReadOnly(key == "序列")
    ...
    def update_tree(self, items: List[Dict[str, Any]]):
        ...
        # Right-click context menu for item actions (Duplicate / Remove / Edit)
        # QTreeView is a QAbstractScrollArea; mouse events fire on the viewport.
        # Connecting on the outer widget can give coordinates that don't match indexAt(),
        # making right-click appear to do nothing.
        self.tree_view.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)  # keep for completeness
        self.tree_view.viewport().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.tree_view.viewport().customContextMenuRequested.connect(self._show_context_menu)
        self.tree_view.viewport().installEventFilter(self)  # robust right-click capture
        self.tree_view.doubleClicked.connect(self._on_double_click_edit)

        # 默认Collapse "Lost Loot" 和 "Equipped"
    ...
    def _get_item_from_index(self, index):
        """Return item dict for a leaf row; None for category/group rows."""
        if not index.isValid():
            return None
        # We store full item dict on the Name column item (column 0)
        ...
        if not isinstance(data, dict):
            return None
        # leaf items generally contain a serial
        if not data.get("serial"):
            return None
        return data

    def eventFilter(self, obj, event):
        # Fallback: some Qt styles/platforms may not emit customContextMenuRequested
        # for QTreeView viewport. Catch the ContextMenu event directly.
        try:
            if obj is self.tree_view.viewport() and event.type() == QEvent.Type.ContextMenu:
    ...
    def _show_context_menu(self, pos):
        # 'pos' should be in viewport coordinates (we connect from viewport),
        # but be defensive if it arrives in widget coordinates.
        vpos = pos
        index = self.tree_view.indexAt(vpos)
        if not index.isValid():
            try:
                vpos = self.tree_view.viewport().mapFrom(self.tree_view, pos)
            except Exception:
                vpos = pos
            index = self.tree_view.indexAt(vpos)
        if not index.isValid():
            return
        item_data = self._get_item_from_index(index)
        if not item_data:
            return
        menu = QMenu(self)
        menu.setStyleSheet("""
        ...
        menu.addSeparator()
        act_edit = menu.addAction("Edit in Wpn Edit")

        chosen = menu.exec(self.tree_view.viewport().mapToGlobal(vpos))
        if chosen == act_dup:
            ...
            self.edit_item_requested.emit(item_data)

    def _on_double_click_edit(self, index):
        item_data = self._get_item_from_index(index)
        if item_data:
            self.edit_item_requested.emit(item_data)
    ...
    def on_item_selected(self, selected, deselected):
        ...
        self.detail_fields["序列"].setPlainText(item_data.get("serial", ""))
        self.detail_fields["解码ID"].setPlainText(item_data.get("decoded_parts", ""))
    ...
    def _on_update_item_clicked(self):
        ...
        new_data = {
            "level": level_val,
            "decoded_parts": self.detail_fields["解码ID"].toPlainText(),
            # 我们不再从UI发送可能过时的序列号
        }
