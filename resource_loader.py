...
def load_json_resource(relative_path: Union[str, Path], 
    ...
    """
    加载JSON资源文件，支持PyInstaller打包环境。结果按路径缓存，减少重复读取。
    """
    path_key = str(relative_path).replace("\\", "/")
    return _load_json_resource_cached(path_key, use_literal_eval)


@lru_cache(maxsize=64)
def _load_json_resource_cached(path_key: str, use_literal_eval: bool) -> Optional[Dict[str, Any]]:
    return _load_json_resource_impl(path_key, use_literal_eval)


def _load_json_resource_impl(relative_path: str, use_literal_eval: bool) -> Optional[Dict[str, Any]]:
    try:
    ...
    except (FileNotFoundError, json.JSONDecodeError, UnicodeDecodeError, Exception):
        return None
...
def load_parts_db() -> Optional[Dict[str, Any]]:
    ...
    if data and isinstance(data.get("rows"), list) and len(data.get("rows", [])) > 0:
        return data
    data = load_json_resource("master_search/db/community_parts_db.json")
    return data
...
def load_text_resource(relative_path: Union[str, Path]) -> Optional[str]:
    ...
    try:
        ...
        # print(f"文件编码Error {relative_path}: {e}")
        return None
    except Exception as e:
        # print(f"加载文本资源时发生UnknownError {relative_path}: {e}")
        return None
...
def load_class_mods_csv(filename: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        print(f"加载CSV文件时发生Error {filename}: {e}")
        return []
...
def load_enhancement_csv(filename: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        print(f"加载Enhancement CSV文件时发生Error {filename}: {e}")
        return []
...
def get_enhancement_data() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"构建Enhancement数据时发生Error: {e}")
        return None
