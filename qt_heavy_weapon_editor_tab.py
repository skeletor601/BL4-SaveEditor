...
def load_heavy_weapon_data(lang='en-US'):
    try:
...
class QtHeavyWeaponEditorTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # When the user manually edits either output box, we stop auto-overwriting their text
        ...
        if self.df_main is None:
            msg = self.ui_loc.get('dialogs', {}).get('load_error', "Error: Heavy weapon data could not be loaded.")
    ...
    def rebuild_output(self, *args):
        ...
        if self._manual_output_mode:
            decoded_now = self.raw_output_edit.toPlainText().strip()
            b85_now = self.b85_output_edit.toPlainText().strip()
        ...
        lists_to_check = []
        if hasattr(self, 'barrel_acc_sel_list'):
            lists_to_check.append(self.barrel_acc_sel_list)
        if hasattr(self, 'body_acc_sel_list'):
            lists_to_check.append(self.body_acc_sel_list)
