...
class ConverterTab(ctk.CTkFrame):
    def __init__(self, parent, main_app_instance):
        ...
        self.batch_add_flag_select = ctk.CTkComboBox(batch_add_controls_frame, values=["1 (普通)", "3 (Favorite)", "5 (Junk)", "17 (Group1)", "33 (Group2)", "65 (Group3)", "129 (Group4)"])
        self.batch_add_flag_select.set("3 (Favorite)")
        ...
        self.yaml_flag_select = ctk.CTkComboBox(yaml_frame, values=["3 (Favorite)", "5 (Junk)", "17 (Group1)", "33 (Group2)", "65 (Group3)", "129 (Group4)"])
        self.yaml_flag_select.set("33 (Group2)")
    ...
    def _generate_and_add_items(self):
        ...
        try:
            ...
            for i, line in enumerate(deserialized_strings):
                ...
                if err:
                    self.main_app.log(f"遍历写入失败 (编码Error): {line} -> {err}")
                    fail_count += 1
                    continue
            ...
            if success_count > 0:
                self.main_app.log(f"遍历写入完成，成功{success_count}个。正在RefreshUI...")
            ...
            self.main_app.log(f"遍历生成并写入时发生Error: {e}")
        finally:
    ...
    def perform_single_conversion(self):
        ...
        try:
            ...
            if error:
                self.single_status.configure(text=f"状态: Error: {error}", text_color="red")
            else:
                target_widget.delete(0, 'end')
                target_widget.insert(0, result)
                self.single_status.configure(text="状态: 转换成功!", text_color="green")
        except Exception as e:
            self.single_status.configure(text=f"状态: 严重Error: {e}", text_color="red")
    ...
    def _process_batch_lines(self, lines):
        ...
        for i, line in enumerate(lines):
            ...
            try:
                ...
                output = result if not error else f"Error: {error}"
            except Exception as e:
                output = f"严重Error: {e}"
    ...
    def _process_iterator(self):
        try:
            ...
            yaml_flag = self.yaml_flag_select.get().split(" ")[0] # "33 (Group2)" -> "33"
            ...
            self.after(0, lambda e=e: self.iterator_status.configure(text=f"状态: 输入Error: {e}", text_color="orange"))
        except Exception as e:
            self.after(0, lambda e=e: self.iterator_status.configure(text=f"状态: 严重Error: {e}", text_color="red"))
        finally:
            self.after(0, self._finalize_iterator_processing)
    ...
    def _process_batch_add(self, lines, flag):
        ...
        for i, line in enumerate(lines):
            ...
            try:
                if self._is_base85(line):
                    ...
                    if err:
                        self.main_app.log(f"批量添加失败 (编码Error): {line} -> {err}")
                        fail_count += 1
                        continue
                ...
                self.main_app.log(f"批量添加时发生严重Error: {e} on line: {line}")
                fail_count += 1
        ...
        def final_update():
            ...
            # 如果有成功添加的，Refresh主界面的物品列表和YAML文本
            if success_count > 0:
                self.main_app.log(f"批量加入背包完成，成功{success_count}个，失败{fail_count}个。正在RefreshUI...")
