...
current_localization_lang = 'en-US'
...
def process_and_load_items(yaml_data: Dict[str, Any]) -> List[ProcessedItem]:
    ...
    for path, item_data in discovered_items:
        ...
        try:
            ...
            print(f"严重解码Error，序列号: {serial}, Error: {e}")
            continue
...
def _find_equipped_node(yaml_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Find equipped-inventory dict by dotted path or recursive search (same pattern as backpack).
    Frontend uses state.equipped_inventory.equipped, state.inventory.equipped, and findNodeByKey('equipped')."""
    for dotted in (
        ...
        node = find_node_by_path(yaml_data, dotted)
        if isinstance(node, dict) and any(isinstance(sk, str) and sk.startswith("slot_") for sk in node.keys()):
            return node
    candidates: List[Dict[str, Any]] = []
    ...
    if candidates:
        return candidates[0]
    # Case-insensitive path search (save keys may vary in casing)
    for path_keys in (
        ["state", "equipped_inventory", "equipped"],
        ["state", "equipped_inventory", "Equipped"],
        ["state", "equipped_inventory", "slots"],
...
def clear_backpack(yaml_data: Dict[str, Any]) -> bool:
    """
    Removes all items from the backpack and equipped (deletes all slot_* keys in both).
    Same logic for both containers. Returns True if backpack was found and cleared, False otherwise.
    """
    backpack_ok = _clear_slot_container(_find_backpack_node(yaml_data))
    _clear_slot_container(_find_equipped_node(yaml_data))
    return backpack_ok
...
def remove_item_by_original_path(yaml_data: dict, original_path):
    ...
    original_path is a list describing nested keys/indices produced by process_and_load_items().
